...
export async function getTodayScheduledMakeups(): Promise<MakeupTicketListResponse> {
  try {
    const supabase = await createClient();
    ...
    return { success: false, error: '오늘 보강 목록을 불러오는데 실패했습니다' };
  }
}
...
// 보강 날짜/시간 예약
export async function scheduleTicket(
  ticketId: string,
  ...
  try {
    ...
    console.error('scheduleTicket error:', error);
    return { success: false, error: '예약에 실패했습니다' };
  }
}

// 보강 취소 (사유 포함)
export async function cancelTicketWithReason(
  ticketId: string,
  reason: string
): Promise<ActionResponse> {
  try {
    ...
    console.error('cancelTicketWithReason error:', error);
    return { success: false, error: '취소 처리에 실패했습니다' };
  }
}

// 보강 취소 (기존 - 사유 없음)
export async function cancelTicket(ticketId: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient();
...
// 대기 상태로 되돌리기
export async function reopenTicket(ticketId: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient();
